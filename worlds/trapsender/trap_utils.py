...
# Creates a dedicated folder at the current execution path
DATA_DIR = Path("TrapSenderData")
DATA_DIR.mkdir(exist_ok=True)

CONFIG_FILE = DATA_DIR / "disabled_traps.json"
...
def load_disabled_traps():
    """Loads disabled traps."""
...
def save_disabled_traps(disabled_set):
    """Saves disabled traps."""
    with open(CONFIG_FILE, 'w') as f:
        json.dump(list(disabled_set), f, indent=4)

# Single shared source of truth for user-toggled-off traps. Lives here
# (not in trap_tab.py) specifically so autoplayer() in Client.py -- which
# runs regardless of whether the GUI ever imports kivy -- can also see it.
# Always mutate this set IN PLACE (add/discard), never reassign the name,
# or modules that did `from .trap_utils import DISABLED_TRAPS` will end up
# holding a stale reference to the old set.
DISABLED_TRAPS = load_disabled_traps()

def is_trap_disabled(name):
    """Checks if trap is disabled."""
    return name in DISABLED_TRAPS

def set_trap_disabled(name, disabled):
    """Sets trap disabled status."""
    if disabled:
        DISABLED_TRAPS.add(name)
    else:
        DISABLED_TRAPS.discard(name)
    save_disabled_traps(DISABLED_TRAPS)
