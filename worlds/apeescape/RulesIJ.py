...
def set_ij_rules(self):
    ...
    connect_regions(self, AERoom.W4L1SecondRoom.value, AELocation.W4L1MaxMahalo.value,
                    lambda state: CB_Lamp(state, self) and (HasHoop(state, self) or HasSling(state, self)) and HasNet(state, self))
    connect_regions(self, AERoom.W4L1SecondRoom.value, AELocation.W4L1Moko.value,
                    lambda state: CB_Lamp(state, self) and (HasSling(state, self) or HasFlyer(state, self)) and HasNet(state, self))
    ...
    connect_regions(self, AERoom.W8L3Tank.value, AERoom.W8L3Boss.value,
                    lambda state: TVT_BossRoom(state, self) and (HasSling(state, self) or (HasFlyer(state, self) and HasRC(state, self))))
...
def CC_5Monkeys(state, world):
    return CrC_Lamp(state, world) and HasNet(state, world) and (HasClub(state, world) or HasSling(state, world) or HasHoop(state, world) or HasFlyer(state, world) or HasPunch(state, world))


def CC_WaterRoom(state, world):
    return (CrC_Lamp(state, world) and CanHitMultiple(state, world)) or (CanSwim(state, world) and (HasFlyer(state, world) or HasPunch(state, world))) or HasFlyer(state, world) or HasHoop(state, world) or HasSling(state, world) or SuperFlyer(state, world)
...
def CP_FrontSewer(state, world):
    return CP_Lamp(state, world) and (HasSling(state, world) or HasRC(state, world))
